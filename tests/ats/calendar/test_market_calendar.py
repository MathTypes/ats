...
def test_next_trading_times_intraday_cme_equity_30m():
    ...
    utc_time = datetime.datetime(2023, 7, 24, 12, 30).astimezone(pytz.timezone("America/New_York"))
...
def test_next_trading_times_near_friday_close_cme_equity_30m():
    ...
    utc_time = datetime.datetime(2023, 7, 21, 19, 30).astimezone(pytz.timezone("America/New_York"))
    logging.info(f"utc_time:{utc_time}")
    
    trading_times = market_time.get_next_trading_times(market_cal, "30M", utc_time, 10)
    # Fri Jul 21 2023 12:30, 13:00:00, 13:15, 14:00
    # Sunday 15:30, 16:00
    assert trading_times == [1689967800.0, 1689969600.0, 1689970500.0, 1689973200.0,
                             1690151400.0, 1690153200.0, 1690155000.0,
                             1690156800.0, 1690158600.0, 1690160400.0]

def test_next_trading_times_near_monday_close_cme_equity_30m():
    market_cal = mcal.get_calendar("CME_Equity")
    # Fri Jul 21 2023 12:30:00
    utc_time = datetime.datetime(2023, 7, 24, 19, 30).astimezone(pytz.timezone("America/New_York"))
    logging.info(f"utc_time:{utc_time}")
    
    trading_times = market_time.get_next_trading_times(market_cal, "30M", utc_time, 10)
    # Mon Jul 24 2023 12:30:00, 13, 13:15
    # Monday 14:00, 15:30, 16:00
    assert trading_times == [1690227000.0, 1690228800.0, 1690229700.0,
                             1690232400.0, 1690237800.0, 1690239600.0,
                             1690241400.0, 1690243200.0, 1690245000.0,
                             1690246800.0]
...
def test_next_trading_times_at_open_cme_equity_30m():
    market_cal = mcal.get_calendar("CME_Equity")
    # Sat
    utc_time = datetime.datetime(2023, 7, 22, 14, 30).astimezone(pytz.timezone("America/New_York"))
    logging.info(f"utc_time:{utc_time}")

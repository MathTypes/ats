...
st.sidebar.write(
    "Analyzing news data to predict stock prices. Understanding the predictive power of the news to predict financial outcomes and generate significant economic impact all over the world."
)

analysis = st.sidebar.radio(
    "Choose analysis",
    ["Data exploration", "Aggregation charts", "Sentiment analysis"],
    index=0,
)
...
def draw_wordcloud(asset="all assets", start_date=None, end_date=None):
    if asset.lower() == "all assets":
        headlines100k = (
            news_df[start_date:end_date]["headline"].str.lower().values[-100000:]
        )
    else:
        headlines100k = (
            news_df.loc[news_df["assetName"] == asset]
            .loc[start_date:end_date, "headline"]
            .str.lower()
            .values[-100000:]
        )
    text = " ".join(headline for headline in headlines100k if type(headline) == str)
...
if analysis.lower() == "data exploration":
    ...
    assetNameGB = market_df[market_df["assetName"] == "Unknown"].groupby("assetCode")
    ...
    with row2_1:
        no_of_drops = st.slider(
            "Select no. of drops to show", min_value=1, max_value=20, value=10, step=1
        )

        grouped = (
            market_df.groupby("time").agg({"price_diff": ["std", "min"]}).reset_index()
        )

        g = grouped.sort_values(("price_diff", "std"), ascending=False)[:no_of_drops]
        g["min_text"] = "Maximum price drop: " + (-1 * g["price_diff"]["min"]).astype(
            str
        )
        data = [
            go.Scatter(
        ...
        st.info(
            "We can see huge price fluctiations when market crashed. But this **must be  wrong**, as there was no huge crash on January 2010... which means that there must be some outliers on our data."
        )
    ...
    with row2_2:
        ...
        time_period = st.date_input(
            "From/To",
            [datetime.date(2007, 7, 6), datetime.date(2009, 7, 8)],
    ...
    if selected_assets:
        ...
        for asset in selected_assets:
            asset_df = market_df[(market_df["assetName"] == asset)].loc[
                start_date:end_date
            ]
        ...
        with st.beta_expander("Description"):
            st.write(
                "We can see some companies' stocks started trading later, some dissappeared. Disappearence could be due to bankruptcy, acquisition or other reasons."
            )
    ...
    def trends_quantile_chart(market_df):
        ...
        for i in [0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95]:
            price_df = (
                market_df.groupby(market_df.index)["close"].quantile(i).reset_index()
            )
    ...
    sentiment_dict = dict(negative="-1", positive="1")

    def top10_mean_sentiment_plot(sentiment, start_date, end_date):

        df_sentiment = news_df.loc[
            news_df["sentimentClass"] == sentiment_dict[sentiment], "assetName"
        ...
        data = go.Bar(
            x=companies,
            y=sentiment_counts,
            marker=dict(color=px.colors.qualitative.Set3[:10]),
        )
    ...
    with row1_1:
        st.plotly_chart(top10_mean_sentiment_plot("negative", start_date, end_date))

    with row1_2:
        st.plotly_chart(top10_mean_sentiment_plot("positive", start_date, end_date))
    ...
    sentiment_df = pd.melt(
        sentiment_df.rename_axis("asset").reset_index(),
        id_vars=["asset"],
        value_vars=sent_labels,
        var_name="sentiment",
        value_name="count",
    )

    fig = px.bar(
        sentiment_df, x="sentiment", y="count", color="asset", barmode="group",
    )
    ...
    for asset in selected_assets:
        ...
        data.append(go.Scatter(x=X, y=Y, name=asset))

...
class _CLIGuestTokenManager(GuestTokenManager):
	...
	def _read(self):
		with self._lock:
			if not os.path.exists(self._file):
				return None
			logging.info(f'Reading guest token from {self._file}')
			with open(self._file, 'r') as fp:
				try:
					o = json.load(fp)
				except json.JSONDecodeError as e:
					logging.warning(f'Malformed guest token file {self._file}: {e!s}')
					self.reset()
					return None
		self._token = o['token']
		self._setTime = o['setTime']
		if self._setTime < time.time() - _GUEST_TOKEN_VALIDITY:
			logging.info('Guest token expired')
			self.reset()

	def _write(self):
		with self._lock:
			logging.info(f'Writing guest token to {self._file}')
	...
	def reset(self):
		super().reset()
		with self._lock:
			logging.info(f'Deleting guest token file {self._file}')
			try:
...
class _TwitterAPIScraper(snscrape.base.Scraper):
	...
	def _ensure_guest_token(self, url = None):
		if self._guestTokenManager.token is None:
			...
			if (match := re.search(r'document\.cookie = decodeURIComponent\("gt=(\d+); Max-Age=10800; Domain=\.twitter\.com; Path=/; Secure"\);', r.text)):
				logging.debug('Found guest token in HTML')
				self._guestTokenManager.token = match.group(1)
			if 'gt' in r.cookies:
				logging.debug('Found guest token in cookies')
				self._guestTokenManager.token = r.cookies['gt']
			if not self._guestTokenManager.token:
				logging.debug('No guest token in response')
				logging.info('Retrieving guest token via API')
		...
		logging.debug(f'Using guest token {self._guestTokenManager.token}')
	...
	def _iter_api_data(self, endpoint, apiType, params, paginationParams = None, cursor = None, direction = _ScrollDirection.BOTTOM):
		...
		emptyPages = 0
		while True:
			logging.info(f'Retrieving scroll page {cursor}')
			...
			if entryCount == 0:
				...
				if self._maxEmptyPages and emptyPages >= self._maxEmptyPages:
					logging.warning(f'Stopping after {emptyPages} empty pages')
					break
			else:
				emptyPages = 0
	...
	def _v2_instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
		if 'tweet' in entry['item']['content']:
			...
			if entry['item']['content']['tweet']['id'] not in obj['globalObjects']['tweets']:
				logging.warning(f'Skipping tweet {entry["item"]["content"]["tweet"]["id"]} which is not in globalObjects')
				return
		...
		if card:
			...
			if hasattr(card, 'url') and '//t.co/' in card.url:
				...
				for u in candidates:
					...
					logging.warning(f'Could not translate t.co card URL on tweet {tweetId}')
		return Tweet(**kwargs)
	...
	def _make_medium(self, medium, tweetId):
		if medium['type'] == 'photo':
			...
			if '.' not in medium['media_url_https']:
				logging.warning(f'Skipping malformed medium URL on tweet {tweetId}: {medium["media_url_https"]!r} contains no dot')
				return
			baseUrl, format = medium['media_url_https'].rsplit('.', 1)
			if format not in ('jpg', 'png'):
				logging.warning(f'Skipping photo with unknown format on tweet {tweetId}: {format!r}')
				return
			mKwargs = {
			...
			logging.warning(f'Unsupported medium type on tweet {tweetId}: {medium["type"]!r}')
	...
	def _make_card(self, card, apiType, tweetId):
		...
		if apiType is _TwitterAPIType.V2:
			...
			for o in card['legacy'].get('user_refs_results', []):
				if 'result' not in o:
					logging.warning(f'Empty user ref object in card on tweet {tweetId}')
					continue
				o = o['result']
				if o['__typename'] == 'UserUnavailable':
					logging.warning(f'Unavailable user in card on tweet {tweetId}')
					continue
				...
				if userId in userRefs:
					if userRefs[userId] != user:
						logging.warning(f'Duplicate user {userId} with differing data in card on tweet {tweetId}')
					continue
		...
		for key, value in messyBindingValues:
			if 'type' not in value:
				...
				if key not in ('creator', 'site'):
					logging.warning(f'Skipping type-less card value {key!r} on tweet {tweetId}')
				continue
			if value['type'] == 'STRING':
				...
				if bindingValues[key] is None:
					logging.warning(f'User {userId} not found in user refs in card on tweet {tweetId}')
			else:
				logging.warning(f'Unsupported card value type on {key!r} on tweet {tweetId}: {value["type"]!r}')
		...
		if cardName in ('summary', 'summary_large_image', 'app', 'direct_store_link_app'):
			...
			if cardName.endswith('_image'):
				...
				if 'vmap' not in bindingValues['player_stream_url']:
					logging.warning(f'Non-VMAP URL in {cardName} player_stream_url on tweet {tweetId}')
			...
			if 'image' in cardName:
				...
				if bindingValues['player_stream_url'] != bindingValues['player_url']:
					if 'vmap' not in bindingValues['player_url']:
						logging.warning(f'Non-VMAP URL in {cardName} player_url on tweet {tweetId}')
			...
			if bindingValues['cta'] not in ctas:
				logging.warning(f'Unsupported message_me card cta on tweet {tweetId}: {bindingValues["cta"]!r}')
				return
			...
			if 'type' in o:
				...
				if unifiedCardType not in (
					...
					logging.warning(f'Unsupported unified_card type on tweet {tweetId}: {unifiedCardType!r}')
					return
				...
				logging.warning(f'Unsupported unified_card type on tweet {tweetId}')
				return
			...
			for k, v in o['component_objects'].items():
				if v['type'] == 'details':
					...
					if not all(b['type'] == 'cta' for b in v['data']['buttons']):
						logging.warning(f'Unsupported unified_card button_group button type on tweet {tweetId}')
						return
					...
					logging.warning(f'Unsupported unified_card component type on tweet {tweetId}: {v["type"]!r}')
					return
				...
				for k, v in o['app_store_data'].items():
					...
					for var in v:
						...
						if 'iconMediumKey' in vKwargs and vKwargs['iconMediumKey'] not in kwargs['media']:
							...
							logging.warning(f'Tweet {tweetId} contains an app icon medium key {vKwargs["iconMediumKey"]!r} on app {vKwargs["type"]!r}/{vKwargs["id"]!r}, but the corresponding medium is missing; dropping')
			...
			if 'layout' in o:
				if o['layout']['type'] == 'swipeable':
					...
					logging.warning(f'Unsupported unified_card layout type on tweet {tweetId}: {o["layout"]["type"]!r}')
					return
			...
			if any(d.appKey not in card.apps for d in card.destinations.values() if d.appKey is not None):
				missingParts.add('apps')
			if missingParts:
				logging.warning(f'Consistency errors in unified card on tweet {tweetId}: missing {", ".join(missingParts)}')

			return card

		logging.warning(f'Unsupported card type on tweet {tweetId}: {cardName!r}')
	...
	def _graphql_timeline_tweet_item_result_to_tweet(self, result, tweetId = None):
		...
		if 'quoted_status_result' in result:
			if 'result' not in result['quoted_status_result']:
				logging.warning(f'quoted_status_result for {tweet["quoted_status_id_str"]} without an actual result on tweet {self._get_tweet_id(tweet)}, using TweetRef')
			...
			if result['quotedRefResult']['result']['__typename'] == 'TweetTombstone':
				...
				if result['quotedRefResult']['result']['__typename'] not in ('Tweet', 'TweetWithVisibilityResults'):
					logging.warning(f'Unknown quotedRefResult type {result["quotedRefResult"]["result"]["__typename"]!r} on tweet {self._get_tweet_id(tweet)}, using TweetRef')
		...
		for instruction in instructions:
			...
			for entry in instruction['entries']:
				if entry['entryId'].startswith('tweet-'):
					...
					logging.warning(f'Skipping unrecognised entry ID: {entry["entryId"]!r}')
	...
	def _user_to_user(self, user, id_ = None):
		...
		if user.get('url'):
			...
			if not entity or entity['url'] != user['url']:
				logging.warning(f'Link inconsistency on user {kwargs["id"]}')
			if not entity:
...
class TwitterUserScraper(TwitterSearchScraper):
	...
	def _get_entity(self):
		...
		if obj['data']['user']['result']['__typename'] == 'UserUnavailable':
			logging.warning('User unavailable')
			return None
		...
		if user['legacy'].get('url'):
			...
			if not entity or entity['url'] != user['legacy']['url']:
				logging.warning(f'Link inconsistency on user')
			if not entity:
...
class TwitterCommunityScraper(_TwitterAPIScraper):
	...
	def _get_entity(self):
		...
		if obj['data']['communityResults']['result']['__typename'] == 'CommunityUnavailable':
			logging.warning('Community unavailable')
			return None

...
def get_time_series_by_instr_date(instr_name, asof_date):
    ...
    file_path = os.path.join(
        config_utils.get_data_root(), assetCode, asof_date.strftime("%Y%m%d") + ".csv"
    )
    try:
...
if __name__ == "__main__":
    ...
    asset = args.instr
    price_df = get_time_series_by_range(asset, args.start_date, args.end_date)
    logging.info(f"price_df:{price_df}")
    for n, g in price_df.groupby(pd.Grouper(freq="M")):
        logging.info(f"n:{n}")
        logging.info(f"g:{g}")
        path_dir = os.path.join(config_utils.get_ts_root(), "monthly", asset)

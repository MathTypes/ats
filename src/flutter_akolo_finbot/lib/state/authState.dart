...
class AuthState extends AppState {
  ...
  void databaseInit() {
    try {
      Utility.logEvent('_profileQuery:' + _profileQuery.toString(),
          parameter: {});
  ...
  Future<String?> signIn(String email, String password,
    ...
    try {
      isBusy = true;
      Utility.logEvent('signIn, email:' + email, parameter: {});
      var result = await _firebaseAuth.signInWithEmailAndPassword(
          email: email, password: password);
      Utility.logEvent('signIn, result:' + result.toString(), parameter: {});
      user = result.user;
      userId = user!.uid;
      return user!.uid;
  ...
  Future<User?> handleGoogleSignIn() async {
    try {
      ...
      Utility.logEvent(
          'handleGoogleSignIn, beforeKAnalytics:' + _googleSignIn.toString(),
          parameter: {});
      kAnalytics.logLogin(loginMethod: 'google_login');
      Utility.logEvent(
          'handleGoogleSignIn, afterKAnalytics:' + _googleSignIn.toString(),
          parameter: {});
      final GoogleSignInAccount? googleUser = await _googleSignIn.signIn();
      Utility.logEvent(
          'handleGoogleSignIn, googleUser:' + googleUser.toString(),
          parameter: {});
      if (googleUser == null) {
      ...
      Utility.logEvent(
          'handleGoogleSignIn, credential:' + credential.toString(),
          parameter: {});
      user = (await _firebaseAuth.signInWithCredential(credential)).user;
      Utility.logEvent('handleGoogleSignIn, user:' + user.toString(),
          parameter: {});
  ...
  Future<String?> signUp(UserModel userModel,
    ...
    try {
      isBusy = true;
      Utility.logEvent('before signup, result:' + userModel.toString(),
          parameter: {});
      var result = await _firebaseAuth.createUserWithEmailAndPassword(
        email: userModel.email!,
        password: password,
      );
      Utility.logEvent('after signup, result:' + userModel.toString(),
          parameter: {});
      user = result.user;
      Utility.logEvent('signup, result:' + result.toString(), parameter: {});
      authStatus = AuthStatus.LOGGED_IN;
  ...
  Future<User?> getCurrentUser() async {
    try {
      ...
      Utility.logEvent('new_get_currentUSer, user:' + user.toString(),
          parameter: {});
      if (user != null) {
        if (user!.email != null)
          Utility.logEvent('get_currentUSer, uid:' + user!.email!,
              parameter: {});
        else
          Utility.logEvent('get_currentUSer, uid:' + user!.uid, parameter: {});
        await getProfileUser();
        Utility.logEvent(
            'after get profile, get_currentUSer:' + user.toString(),
            parameter: {});
        authStatus = AuthStatus.LOGGED_IN;
        userId = user!.uid;
      } else {
  ...
  Future<String> _uploadFileToStorage(File file, path) async {
    Utility.logEvent('_uploadFileToStorage, path:' + path.toString(),
        parameter: {});
  ...
  void updateFCMToken() {
    ...
    Utility.logEvent('updateFCMToken', parameter: {});
    getProfileUser();
    _firebaseMessaging.getToken().then((String? token) {
      Utility.logEvent('updateFCMToken, toke:' + token.toString(),
          parameter: {});
      assert(token != null);

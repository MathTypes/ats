...
def get_contract_data(contract_ids=()):
    ...
    for con_id in contract_ids:
        contract_details = ib_gw.reqContractDetails(Contract(conId=con_id))[
            0
        ].nonDefaults()
...
def get_signal(identifier):
    ...
    try:
        ...
        logger.error(
            "Request to service {} returned an exception: {}".format(identifier, e)
        )
        raise e
        # return {}
    except (
...
def main():
    ...
    config = (
        ...
        .document("paper" if TRADING_MODE == "local" else TRADING_MODE)
        .get()
        .to_dict()
    )
    ...
    activity_log = {
        "agent": (
            re.match("([\\w-]+)-([0-9]+|manual-[0-9a-z]+)-[0-9a-z]+$", HOSTNAME)
        ).group(1)
    ...
    try:
        ...
        signals = {
            s: get_signal("localhost" if TRADING_MODE == "local" else s)
            for s in strategies
        }
        ...
        positions = {
            ...
            for item in ib_gw.positions(config["account"])
        }
        activity_log["positions"] = positions

        # get contract details
        contract_data = get_contract_data(
            set(list(allocation.keys()) + list(positions.keys()))
        )
        # build contractId->symbol lookup dict
        symbol_map = {k: v["contract"].localSymbol for k, v in contract_data.items()}
        activity_log["contractIds"] = {
            v["contract"].localSymbol: k for k, v in contract_data.items()
        }
        # replace dict keys
        activity_log["signals"] = {
            k: {symbol_map[k]: v for k, v in v.items()} for k, v in signals.items()
        }
        activity_log["scaledSignals"] = [
            {symbol_map[k]: v for k, v in s.items()} for s in scaled_signals
        ]
                ...
                * net_liquidation
                / (
                    ...
                    * (
                        ...
                        if fx[contract_data[k]["contract"].currency].midpoint()
                        == fx[contract_data[k]["contract"].currency].midpoint()
        ...
        activity_log["positions"] = {symbol_map[k]: v for k, v in positions.items()}
        activity_log["targetPositions"] = {
            symbol_map[k]: v for k, v in target_positions.items()
        }

        # calculate trade
        trades = {
            k: target_positions[k] - positions[k] for k in target_positions.keys()
        }
        ...
        if not DRY_RUN:
            ...
            for k, v in trades.items():
                order = ib_gw.placeOrder(
                    contract_data[k]["contract"],
                    MarketOrder(
                        action="BUY" if v > 0 else "SELL", totalQuantity=abs(v)
                    ).update(**order_properties),
                )

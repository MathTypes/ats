...
class TestGcpModule(unittest.TestCase):
    ...
    def test_bigquery(self):
        ...
        data = DataFrame(
            {"date": [dt], "instrument": ["test"], "key": [key], "value": [42.0]}
        )
        load_job = self.gcp_module.bq.load_table_from_dataframe(
            data, self.BIGQUERY_DESTINATION, job_config=self.BIGQUERY_JOB_CONFIG
        )

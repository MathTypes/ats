...
class TestCloseAll(unittest.TestCase):
    ...
    ENV = {"K_REVISION": "k_revision", "ORDER_PROPERTIES": '{"goodAfterTime":"today"}'}
    ...
    @patch(
        "intents.close_all.Trade",
        return_value=MagicMock(
            consolidate_trades=MagicMock(),
            place_orders=MagicMock(return_value="orders"),
            trades={
                "abc": {"contract": MagicMock(local_symbol="ABC"), "quantity": 100},
                "def": {"contract": MagicMock(local_symbol="DEF"), "quantity": -10},
            },
        ),
    )
    def test_core(self, trade, market_order):
        strategy_side_effect = [
            MagicMock(
                contracts={
                    "abc": MagicMock(
                        contract=MagicMock(localSymbol="ABC"), local_symbol="ABC"
                    ),
                    "def": MagicMock(
                        contract=MagicMock(localSymbol="DEF"), local_symbol="DEF"
                    ),
                },
            ...
            MagicMock(
                contracts={
                    ...
                    "ghi": MagicMock(
                        contract=MagicMock(localSymbol="GHI"), local_symbol="GHI"
                    ),
                },
        ...
        with patch.object(
            ...
            "_env",
            db=MagicMock(
                document=MagicMock(return_value=MagicMock(delete=MagicMock())),
                collection=MagicMock(
                    return_value=MagicMock(
                        get=MagicMock(
                            return_value=[MagicMock(id=f"s{i + 1}") for i in range(2)]
                        )
                    )
                ),
            ),
            ibgw=MagicMock(
                orders=MagicMock(
                    return_value=[MagicMock(permId=f"o{i}") for i in range(3)]
                ),
                cancelOrder=MagicMock(),
                portfolio=MagicMock(
                    return_value=[
                        MagicMock(contract=MagicMock(conId="abc"), position=-100),
                        MagicMock(contract=MagicMock(conId="def"), position=10),
                    ]
                ),
            ),
            logging=MagicMock(warning=MagicMock()),
        ) as env:
            with patch(
                "intents.close_all.Strategy", side_effect=strategy_side_effect
            ) as strategy:
                expected_log = {
                ...
                try:
                    env.ibgw.cancelOrder.assert_has_calls(
                        [call(m) for m in env.ibgw.orders.return_value]
                    )
                    env.db.document.assert_has_calls(
                        [
                            call(
                                f"positions/{self.test_obj._env.trading_mode}/openOrders/o{i}"
                            )
                            for i in range(3)
                        ]
                    )
                    self.assertEqual(3, env.db.document.return_value.delete.call_count)
                    env.db.collection.assert_called_once_with(
                        f"positions/{self.test_obj._env.trading_mode}/holdings"
                    )
                    strategy.assert_has_calls(
                        [
                            call(k.id)
                            for k in env.db.collection.return_value.get.return_value
                        ]
                    )
                    trade.return_value.consolidate_trades.assert_called_once()
                    trade.return_value.place_orders.assert_called_once_with(
                        market_order, order_properties=self.test_obj._order_properties
                    )
            ...
            with patch("intents.close_all.Strategy", side_effect=strategy_side_effect):
                with patch(
                    ...
                    return_value=MagicMock(
                        trades={
                            "abc": {
                                "contract": MagicMock(local_symbol="ABC"),
                                "quantity": -100,
                            },
                            "def": {
                                "contract": MagicMock(local_symbol="DEF"),
                                "quantity": 10,
                            },
                        }
                    ),
                    ...
                    try:
                        portfolio = {
                            ...
                            for item in env.ibgw.portfolio()
                        }
                        env.logging.warning.assert_called_once_with(
                            f"Consolidated trade and IB portfolio don't match - portfolio: {portfolio}"
                        )
                    except AssertionError:
                        self.fail()

...
class TestAllocation(unittest.TestCase):
    ...
    ACTIVITY_LOG = {"agent": "k_revision", "exception": None}
    CONFIG = {
        "account": 123,
        "adaptivePriority": "adaptive_priority",
        "exposure": {"strategies": {"s1": 0.5, "s2": 0.25}, "overall": 1},
        "retryCheckMinutes": 0,
    }
            ...
            db=MagicMock(
                document=MagicMock(
                    return_value=MagicMock(
                        get=MagicMock(
                            return_value=MagicMock(
                                to_dict=MagicMock(return_value=CONFIG)
                            )
                        )
                    )
                )
    ...
    def test_core(self, trade, market_order, tag_value):
        with patch.object(
            ...
            env=self.ENV,
            get_account_values=MagicMock(
                return_value={"NetLiquidation": {"CHF": 12345}}
            ),
        ) as env:
            with patch.object(
                self.test_obj, "_strategies", self.STRATEGIES
            ) as strategies:
                expected_log = {
                ...
                try:
                    env.get_account_values.assert_called_once_with(
                        self.CONFIG["account"]
                    )
                    for k, v in strategies.items():
                        v.assert_called_once_with(
                            base_currency=[
                                *env.get_account_values.return_value[
                                    "NetLiquidation"
                                ].keys()
                            ][0],
                            exposure=[
                            ...
                            * self.CONFIG["exposure"]["overall"]
                            * self.CONFIG["exposure"]["strategies"][k],
                        )
                    trade.assert_called_with(
                        [v.return_value for v in self.test_obj._strategies.values()]
                    )
                    ...
                    trade.return_value.place_orders.assert_called_once_with(
                        ...
                        order_properties={
                            **self.test_obj._order_properties,
                            "tif": "DAY",
                        },
                    )
                    tag_value.assert_called_once_with(
                        "adaptivePriority", self.CONFIG["adaptivePriority"]
                    )
                ...
                env.config = {
                    **self.CONFIG,
                    "exposure": {"strategies": {"s1": 0, "s2": 0.25}, "overall": 1},
                }

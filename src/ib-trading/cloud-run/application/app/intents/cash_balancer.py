...
class CashBalancer(Intent):
    ...
    def _core(self):
        ...
        account_values = self._env.get_account_values(
            self._env.config["account"],
        ...
        trades = {
            k
            + base_currency: -(
                int(v / account_values["ExchangeRate"][k]) // 1000 * 1000
            )
        ...
        if not self._dry_run and len(trades):
            ...
            orders = {
                ...
                if t.orderStatus.permId in perm_ids
            }

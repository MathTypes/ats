...
class TestVxcurve(unittest.TestCase):
    ...
    def test_get_signals(self, _register_contracts, randint):
        instruments = {"mnq": [MagicMock(contract=MagicMock(conId="mnq"))]}

        with patch.object(self.test_obj, "_instruments", instruments):
            expected = {instruments["mnq"][0].contract.conId: randint.return_value}
    ...
    @patch(
        "strategies.dummy.Future.get_contract_series", return_value="contract-series"
    )
    def test_setup(self, get_contract_series):
        expected = {"mnq": get_contract_series.return_value}
        self.test_obj._setup()
        self.assertDictEqual(expected, self.test_obj._instruments)
        try:
            get_contract_series.assert_called_once_with(
                1, "MNQ", rollover_days_before_expiry=2
            )
        except AssertionError:
            self.fail()

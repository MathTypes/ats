...
class BarData(Object):
    ...
    def __str__(self):
        return (
            "Date: %s, Open: %s, High: %s, Low: %s, Close: %s, Volume: %s, WAP: %s, BarCount: %s"
            % (
                self.date,
...
class HistogramData(Object):
    ...
    def __str__(self):
        return "Price: %s, Size: %s" % (
            ibapi.utils.floatMaxString(self.price),
    ...
    def __str__(self):
        ...
        return (
            ...
            % (
                ...
                self.listingExch,
                self.serviceDataType,
                ibapi.utils.intMaxString(aggGroup),
            )
        )
...
class PriceIncrement(Object):
    ...
    def __str__(self):
        return "LowEdge: %s, Increment: %s" % (
            ibapi.utils.floatMaxString(self.lowEdge),
            ibapi.utils.floatMaxString(self.increment),
        )
...
class HistoricalTickBidAsk(Object):
    ...
    def __str__(self):
        return (
            "Time: %s, TickAttriBidAsk: %s, PriceBid: %s, PriceAsk: %s, SizeBid: %s, SizeAsk: %s"
            % (
                ibapi.utils.intMaxString(self.time),
                self.tickAttribBidAsk,
                ibapi.utils.floatMaxString(self.priceBid),
                ibapi.utils.floatMaxString(self.priceAsk),
                ibapi.utils.decimalMaxString(self.sizeBid),
                ibapi.utils.decimalMaxString(self.sizeAsk),
            )
        )
...
class HistoricalTickLast(Object):
    ...
    def __str__(self):
        return (
            "Time: %s, TickAttribLast: %s, Price: %s, Size: %s, Exchange: %s, SpecialConditions: %s"
            % (
                ibapi.utils.intMaxString(self.time),
                self.tickAttribLast,
                ibapi.utils.floatMaxString(self.price),
...
class WshEventData(Object):
    ...
    def __str__(self):
        return (
            "WshEventData. ConId: %s, Filter: %s, Fill Watchlist: %d, Fill Portfolio: %d, Fill Competitors: %d"
            % (
                ibapi.utils.intMaxString(self.conId),
                self.filter,
                self.fillWatchlist,
                self.fillPortfolio,
                self.fillCompetitors,
            )
        )

...
def EODisparity(n_eyz, each_z=False):
    ...
    if not each_z:
        eod = 0
        p11 = sum([n_eyz[(1, 1, z)] for z in z_set]) / sum(
            [n_eyz[(1, 1, z)] + n_eyz[(0, 1, z)] for z in z_set]
        )
        for z in z_set:
            try:
                eod_z = abs(
                    n_eyz[(1, 1, z)] / (n_eyz[(0, 1, z)] + n_eyz[(1, 1, z)]) - p11
                )
...
def makeEffortPGD(
    model, features, u_index, C, C_identity, delta=0.3, lr=0.005, iters=10, efforts=None
):
    ...
    for i in range(iters):
        ...
        cost = F.cross_entropy(
            logits,
            torch.ones(features.shape[0]).type(torch.LongTensor),
            reduction="sum",
        ).to(DEVICE)
        model.zero_grad()
        cost.backward()

        y = efforts - (lr / ((i + 1) ** 0.5)) * efforts.grad
...
def loss_dp_func(option, logits, targets, sensitive, larg=1):
    ...
    fair_loss0 = torch.mul(
        sensitive - sensitive.type(torch.FloatTensor).mean(),
        logits.T[0] - torch.mean(logits.T[0]),
    )
    fair_loss0 = torch.mean(torch.mul(fair_loss0, fair_loss0))
    fair_loss1 = torch.mul(
        sensitive - sensitive.type(torch.FloatTensor).mean(),
        logits.T[1] - torch.mean(logits.T[1]),
    )
    ...
    if option == "local zafar":
        ...
        acc_loss_prime = F.cross_entropy(
            logits,
            torch.ones(logits.shape[0]).type(torch.LongTensor).to(DEVICE),
            reduction="sum",
        )
...
def loss_func(option, probas, targets, logits, sensitive, larg=1):
    ...
    acc_loss = F.binary_cross_entropy(
        probas.T[1], targets.type(torch.FloatTensor), reduction="sum"
    )
    fair_loss0 = torch.mul(
        sensitive - sensitive.type(torch.FloatTensor).mean(),
        logits.T[0] - torch.mean(logits.T[0]),
    ...
    if option == "local zafar":
        ...
        # acc_loss = torch.sum(torch.nn.BCELoss(reduction = 'none')((outputs.T[1]+1)/2, torch.ones(logits.shape[0])))
        acc_loss_prime = F.binary_cross_entropy(
            probas.T[1],
            torch.ones(probas.shape[0]).type(torch.FloatTensor).to(DEVICE),
            reduction="sum",
        )
        return acc_loss_prime, acc_loss, fair_loss
    else:
...
def eo_loss(
    logits, targets, sensitive, larg, mean_z1=None, left=None, option="local fc"
):
    acc_loss = F.cross_entropy(logits, targets, reduction="sum")
    ...
    if left:
        ...
        fair_loss0 = torch.mul(
            sensitive[y1_idx] - sensitive.type(torch.FloatTensor).mean(),
            logits.T[0][y1_idx] - torch.mean(logits.T[0][y1_idx]),
        )
        fair_loss0 = torch.mean(torch.mul(fair_loss0, fair_loss0))
        fair_loss1 = torch.mul(
...
def zafar_loss(logits, targets, outputs, sensitive, larg, mean_z, left):
    acc_loss = F.cross_entropy(logits, targets, reduction="sum")
    fair_loss = torch.mean(
        torch.mul(sensitive - mean_z, logits.T[0] - torch.mean(logits.T[0]))
    )
    if left:
        return acc_loss - larg * fair_loss
    else:
...
def mtl_loss(logits, targets, penalty, global_model, model):
    penalty_term = torch.tensor(0.0, requires_grad=True).to(DEVICE)
    ...
    loss = (
        F.cross_entropy(logits, targets, reduction="sum") + penalty / 2 * penalty_term
    )
    return loss
...
########################
X_DIST = {
    0: {"mean": (-2, -2), "cov": np.array([[10, 1], [1, 3]])},
...
def X_PRIME(x):
    return (
        x[0] * np.cos(np.pi / 4) - x[1] * np.sin(np.pi / 4),
...
def Z_MEAN(x):
    ...
    return multivariate_normal.pdf(
        x_transform, mean=X_DIST[1]["mean"], cov=X_DIST[1]["cov"]
    ) / (
        multivariate_normal.pdf(
            x_transform, mean=X_DIST[1]["mean"], cov=X_DIST[1]["cov"]
        )
        + multivariate_normal.pdf(
            x_transform, mean=X_DIST[0]["mean"], cov=X_DIST[0]["cov"]
        )
    )


def dataSample(train_samples=1400, test_samples=600, y_mean=0.6, Z=2, seed=123):
    np.random.seed(seed)
    ...
    if Z == 2:
        for y in ys:
            x = np.random.multivariate_normal(
                mean=X_DIST[y]["mean"], cov=X_DIST[y]["cov"], size=1
            )[0]
        ...
        for y in ys:
            ...
            py1 = multivariate_normal.pdf(
                x, mean=X_DIST[1]["mean"], cov=X_DIST[1]["cov"]
            )
            py0 = multivariate_normal.pdf(
                x, mean=X_DIST[0]["mean"], cov=X_DIST[0]["cov"]
            )
    ...
    data = pd.DataFrame(
        zip(np.array(xs).T[0], np.array(xs).T[1], ys, zs),
        columns=["x1", "x2", "y", "z"],
    )
...
def gapData(
    train_samples=1400, test_samples=600, seed=123, gap_y=0.1, y_mean=0.3, gap_z=0.1
):
    np.random.seed(seed)
    num_samples = train_samples + test_samples
    ys = np.random.binomial(n=1, p=y_mean, size=num_samples)
    ...
    for y in ys:
        ...
        x_dist = {
            (1, 1): {
                "mean": (3.8 + gap_z, 2 + gap_z * 5),
                "cov": np.array([[0.05, 0.01], [0.01, 0.02]]),
            },
            (1, 0): {"mean": (3.8, 2), "cov": np.array([[0.05, 0.01], [0.01, 0.02]])},
            (0, 0): {
                "mean": (3.8 - 1.5 * gap_y, 2 - 1.5 * gap_y * 5),
                "cov": np.array([[0.1, 0.01], [0.01, 0.03]]),
            },
            (0, 1): {
                "mean": (3.8 - gap_y + gap_z, 2 - gap_y * 5 + gap_z * 5),
                "cov": np.array([[0.1, 0.01], [0.01, 0.03]]),
            },
        }
        x = np.random.multivariate_normal(
            mean=x_dist[(y, z)]["mean"], cov=x_dist[(y, z)]["cov"], size=1
        )[0]
        xs.append(x)
        zs.append(z)
    ...
    if len(sensitive_attributes) > 1:
        if privileged_classes != None:
            for i in range(len(sensitive_attributes)):
                df.loc[
                    df[sensitive_attributes[i]] != privileged_classes[i],

    ...
    def add_model_predictions(self, epoch, logs=None):
        ...
        for idx in table_idxs:
            ...
            self.pred_table.add_data(
                ...
                - self.data_table_ref.data[idx][9],  # error_max
                pred[2]
                - self.data_table_ref.data[idx][11]
                - self.data_table_ref.data[idx][10],  # error_min
                # pred[4], # img_interp
                pred[4],  # rmse
                pred[5],  # mae
            )
        ...
        for batch_idx in range(self.num_samples):
            ...
            for idx in range(len(y_hats)):
                ...
                prediction_date_time = (
                    ...
                    + " "
                    + str(self.data_table_ref.data[idx][12])
                    + " "
                    + day_of_week_map[self.data_table_ref.data[idx][3]]
                )
                fig.update_layout(title=prediction_date_time, font=dict(size=20))

...
def pull_futures_sample_data(raw_dir:str, ticker: str, asset_type: str) -> pd.DataFrame:
    #ticker = ticker.replace("CME_","")
    names = ["Time", "Open", "High", "Low", "Close", "Volume"]
    if asset_type in ["FUT"]:
        file_path = os.path.join(f"{raw_dir}/futures",
                                 f"{ticker}_1min_continuous_adjusted.txt")
    else:
        file_path = os.path.join(f"{raw_dir}/stock",
...
df["Time"] = df["Time"].apply(lambda x: x.to_pydatetime().replace(tzinfo=newYorkTz).astimezone(pytz.timezone("UTC")))
...
id_column = "ticker"
sort_column = "time"
rolled_backward = roll_time_series(df,
                                   column_id=id_column,

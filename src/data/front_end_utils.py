...
def get_list_ner(df):
    ...
    for i in range(df.shape[0]):
        text_ner.append(
            list(df["text_ner_names"].iloc[i][1:-1].replace("'", "").split(", "))
        )
...
def custom_visualize_ner(
    ...
    *,
    ...
    st.components.v1.html(
        f"{style}{get_html(html)}", width=1400, height=500, scrolling=True
    )
...
def display_text(df, list_ner, text_ner, text_ner_count):
    for ner in list_ner:
        ind = sorted(
            [
                match
                for match in find(ner, text_ner=text_ner, text_ner_count=text_ner_count)
            ],
            key=lambda x: x[2],
            reverse=True,
        )[0]

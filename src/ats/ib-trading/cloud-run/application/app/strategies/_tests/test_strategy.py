...
class TestStrategy(unittest.TestCase):
    ...
    def test_init(
        ...
        *_,
        ...
        with patch(
            "strategies.strategy.Strategy._signals", {i: i * 10 for i in [1, 2, 3]}
        ):
            with patch(
                "strategies.strategy.Strategy._holdings",
                {i: i * 10 for i in [2, 3, 4, 5]},
            ):
                strategy = Strategy(**kwargs)
    ...
    def test_calculate_target_positions(self, _get_currencies):
        contracts = {
            "abc": MagicMock(
                contract=MagicMock(
                    currency="USD", multiplier=2, get_tickers=MagicMock()
                ),
                tickers=MagicMock(close=10),
            ),
            "def": MagicMock(
                contract=MagicMock(
                    currency="EUR", multiplier=5, get_tickers=MagicMock()
                ),
                tickers=MagicMock(close=20),
            ),
            "ghi": MagicMock(
                contract=MagicMock(
                    currency="CHF", multiplier=10, get_tickers=MagicMock()
                ),
                tickers=MagicMock(close=5),
            ),
        }

        with patch.object(
            self.test_obj, "_signals", {"abc": 1.2, "def": -2.3, "ghi": 3.4}
        ):
                    ...
                    with patch.object(self.test_obj, "_base_currency", "CHF"):
                        with patch.object(self.test_obj, "_contracts", contracts):
                            ...
                            self.assertDictEqual(
                                {"abc": 3333, "def": -1045, "ghi": 3400},
                                self.test_obj._target_positions,
                            )
                            try:
                        ...
                        type(contracts["ghi"]).tickers = PropertyMock(
                            side_effect=[None, MagicMock(close=5)]
                        )
                        with patch.object(self.test_obj, "_contracts", contracts):
                            self.test_obj._calculate_target_positions()
                            self.assertDictEqual(
    ...
    def test_calculate_trades(self):
        with patch.object(
            self.test_obj, "_target_positions", {"abc": 1, "def": 2, "ghi": 3}
        ):
            with patch.object(
                self.test_obj, "_holdings", {"abc": 2, "def": 1, "ghi": 3}
            ):
                with patch.object(
                    self.test_obj,
                    "_contracts",
                    {k: MagicMock(local_symbol=k) for k in ["abc", "def", "ghi"]},
                ):
    ...
    def test_get_currencies(self):
        ...
        with patch(
            "strategies.strategy.InstrumentSet", MagicMock(get_tickers=MagicMock())
        ) as instrumentset:
            instrumentset.return_value.__iter__.return_value = [
                MagicMock(
                    get_ticker=MagicMock(),
                    tickers=MagicMock(close=1, midpoint=MagicMock(return_value=1)),
                ),
                MagicMock(
                    get_ticker=MagicMock(),
                    tickers=MagicMock(
                        close=1, midpoint=MagicMock(return_value=float("nan"))
                    ),
                ),
                MagicMock(
                    get_ticker=MagicMock(),
            ...
            with patch("strategies.strategy.Forex") as forex:
                with patch.object(
                    self.test_obj,
                    "_contracts",
                    {c: MagicMock(contract=MagicMock(currency=c)) for c in currencies},
                ):
                    self.test_obj._get_currencies(base_currency)
                    self.assertDictEqual(
                        {"CHF": 1, "EUR": 1, "USD": 1}, self.test_obj._fx
                    )
                    try:
                        forex.assert_has_calls(
                            [call(pair=c + base_currency) for c in set(currencies)]
                        )
    ...
    def test_get_holdings(self, _register_contracts):
        with patch.object(
                ...
                document=MagicMock(
                    return_value=MagicMock(
                        get=MagicMock(
                            return_value=MagicMock(
                                to_dict=MagicMock(
                                    return_value={f"{i}": i for i in range(3)}
                                ),
                                exists=True,
                            )
                        )
            ...
            try:
                env.db.document.assert_called_once_with(
                    "positions/trading_mode/holdings/strategy"
                )
    ...
    def test_register_contracts(self):
        self.assertRaises(
            TypeError,
            self.test_obj._register_contracts,
            1,
            "a",
        ...
        with patch.object(self.test_obj, "_contracts", {1: "1", 2: "2"}):
            with patch("strategies.strategy.Contract") as contract:
                ...
                self.assertDictEqual(
                    {1: "1", 2: "2", 3: contract.return_value}, self.test_obj._contracts
                )
                try:
        ...
        with patch.object(self.test_obj, "_contracts", {1: "1", 2: "2"}):
            contracts = [
                MagicMock(spec=Instrument, contract=MagicMock(conId=2)),
                MagicMock(spec=Instrument, contract=MagicMock(conId=3)),
            ]
            self.test_obj._register_contracts(*contracts)
            self.assertDictEqual(
                {1: "1", 2: "2", 3: contracts[1]}, self.test_obj._contracts
            )

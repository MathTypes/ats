...
class TradeReconciliation(Intent):
    ...
    def _core(self):
        ...
        for fill in self._env.ibgw.fills():
            ...
            query = self._env.db.collection(
                f"positions/{self._env.trading_mode}/openOrders"
            ...
            if len(order) and side * fill.execution.cumQty == sum(
                order["source"].values()
            ):
                fills.append(
                    {
                ...
                for strategy, quantity in order["source"].items():
                    holdings_doc = self._env.db.document(
                        f"positions/{self._env.trading_mode}/holdings/{strategy}"
                    )
                    ...
                    with self._env.db.transaction() as tx:
                        ...
                        action(
                            holdings_doc,
                            {str(contract_id): position + quantity or DELETE_FIELD},
                        )
                        tx.delete(order_doc)
        ...
        self._activity_log.update(
            portfolio={
                item.contract.localSymbol: item.position for item in ib_portfolio
            }
        )
        holdings = [
            doc.get().to_dict()
        ...
        self._activity_log.update(
            consolidatedHoldings={
                self._env.ibgw.reqContractDetails(Contract(conId=k))[
                    0
        ...
        if portfolio != holdings_consolidated:
            self._env.logging.warning(
                f"Holdings do not match -- Firestore: {holdings_consolidated}; IB: {portfolio}"
            )
            raise AssertionError(
                "Holdings in Firestore do not match the ones in IB portfolio."
            )

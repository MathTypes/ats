        ...
        with patch.object(
            ...
            ibgw=MagicMock(
                placeOrder=MagicMock(
                    return_value=MagicMock(order=MagicMock(permId="permId"))
                ),
                trades=MagicMock(
                    return_value=[
                        MagicMock(
                            contract=MagicMock(pair=MagicMock(return_value="pair")),
                            order=MagicMock(
                                nonDefaults=MagicMock(return_value={"a": "A"})
                            ),
                            orderStatus=MagicMock(
                                permId="permId",
                                nonDefaults=MagicMock(return_value={"b": "B"}),
                            ),
                        )
                    ]
                ),
            ...
            try:
                env.get_account_values.assert_called_once_with(
                    "account", rows=["NetLiquidation", "CashBalance", "ExchangeRate"]
                )
                env.ibgw.placeOrder.assert_called_once_with(
                    forex.return_value, market_order.return_value
                )
            ...
            env.config = {
                "account": "account",
                "cashBalanceThresholdInBaseCurrency": 20000,
            }
            expected_log = {
                **activity_log,
                "exposure": {"EUR": -1172.5, "USD": 6912.0},

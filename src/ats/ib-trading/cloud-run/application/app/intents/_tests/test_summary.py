...
class TestSummary(unittest.TestCase):
    ...
    def test_core(self, get_positions, get_trades, get_fills):
        with patch.object(
            ...
            get_account_values=MagicMock(return_value="abc"),
        ) as env:
            expected = {
                "accountSummary": "abc",
                "portfolio": "def",
        ...
        with patch.object(
            ...
            ibgw=MagicMock(
                fills=MagicMock(
                    return_value=[
                        MagicMock(
                            contract=MagicMock(localSymbol=f"s{i}"),
                            execution=MagicMock(
                                shares=(i + 1) * 10,
                                price=(i + 1) * 2.73,
                                side=f"side{i}",
                                cumQty=(i + 1) * 100,
                                avgPrice=(i + 1) * 3.14,
                                time=datetime(2021, 9, i + 1, 12, 34, 56),
                            ),
                            commissionReport=MagicMock(
                                commission=(i + 1) * 0.1234,
                                realizedPNL=(i + 1) * 1.2345,
                            ),
                        )
                        for i in range(2)
                    ]
                ...
                openTrades=MagicMock(
                    return_value=[
                        MagicMock(
                            ...
                            order=MagicMock(
                                action=f"a{i}",
                                totalQuantity=float(i + 1),
                                orderType=f"t{i}",
                                lmtPrice=(i + 1) * 123,
                                tif="tif{i}",
                                goodAfterTime=f"gat{i}",
                                goodTillDate=f"gtd{i}",
                            ),
                            orderStatus=MagicMock(status=f"s{i}", whyHeld=f"wh{i}"),
                        )
                        for i in range(2)
                    ]
                )

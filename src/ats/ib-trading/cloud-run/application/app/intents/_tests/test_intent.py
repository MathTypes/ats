...
class TestIntent(unittest.TestCase):
    ...
    @patch(
        "intents.intent.Environment",
        return_value=MagicMock(config=CONFIG, env=ENV, trading_mode=TRADING_MODE),
    )
    def test_init(self, env):
        ...
        expected = {
            **self.ACTIVITY_LOG,
            ...
            "intent": "Intent",
            "signature": md5(
                b'k_revisionIntent{"abc": "abc", "xyz": "xyz"}'
            ).hexdigest(),
    ...
    def test_core(self):
        with patch.object(
            self.test_obj, "_env", ibgw=MagicMock(), logging=MagicMock()
        ) as env:
    ...
    @patch(
        "intents.intent.datetime",
        utcnow=MagicMock(return_value=datetime(1977, 9, 27, 19, 15)),
    )
            ...
            db=MagicMock(
                collection=MagicMock(
                    return_value=MagicMock(
                        document=MagicMock(
                            return_value=MagicMock(
                                set=MagicMock(side_effect=[None, Exception])
                            )
                        )
                    )
                )
        ...
        with patch.object(
            ...
            with patch.object(self.test_obj, "_activity_log", activity_log):
                ...
                self.assertDictEqual(
                    {**activity_log, "timestamp": dt.utcnow.return_value},
                    self.test_obj._activity_log,
                )
                try:
                    env.db.collection.return_value.document.return_value.set.assert_called_once_with(
                        self.test_obj._activity_log
                    )
                except AssertionError:
                    self.fail()
    ...
    def test_run(self, core, log_activity):
        ...
        with patch.object(
            ...
            try:
                ...
                env.ibgw.reqMarketDataType.assert_called_once_with(
                    self.CONFIG["marketDataType"]
                )
            ...
            actual = self.test_obj.run()
            self.assertDictEqual(
                {**activity_log, "timestamp": activity_log["timestamp"].isoformat()},
                actual,
            )

            self.assertRaises(Exception, self.test_obj.run)
            self.assertDictEqual(
                {**activity_log, "exception": "Exception: error"},
                self.test_obj._activity_log,
            )
            try:
                env.logging.error.assert_called_once()

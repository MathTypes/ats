...
CONTRACT_SPECS = {
    "ES": {"months": ["H", "M", "U", "Z"], "exchange": "GLOBEX", "currency": "USD"}
}
...
def get_contracts(series, n):
    ...
    contract_symbols = [
        series + m + y for y in contract_years for m in CONTRACT_SPECS[series]["months"]
    ]
    ...
    contracts = {
        ...
        if v is not None
        and v.lastTradeDateOrContractMonth
        > (datetime.now() + timedelta(days=6)).strftime("%Y%m%d")
    }

    return [
        contracts[k] for k in [k for k in contract_symbols if k in contracts.keys()][:n]
    ]
...
def get_prices(contracts=()):
    ...
    return [
        ticker.close if not isnan(ticker.close) else ticker.last
        for ticker in ib_gw.reqTickers(*contracts)
    ]

...
import numpy as np
import os
from settings.settings import SINGLE_TICKER
...
def resample_dataset_timeframe(df, freq: str):
    ...
    df2 = pd.DataFrame()
    df2["high"] = (
        df[["timestamp_5day", "high"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .max()
    )
    df2["low"] = (
        df[["timestamp_5day", "low"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .min()
    )
    df2["open"] = (
        df[["timestamp_5day", "open"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .first()
    )
    df2["close"] = (
        df[["timestamp_5day", "close"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .last()
    )
    df2["volume"] = (
        df[["timestamp_5day", "volume"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .sum()
    )
    df2["ticker"] = (
        df[["timestamp_5day", "ticker"]]
        .groupby(pd.Grouper(key="timestamp_5day", axis=0, freq=freq))
        .first()
    )
    df = df2.dropna()
...
def plot_time_divs(df, new_timeframe: str):
    ...
    if new_timeframe == "daily":
        df["ny_hour"] = df.index.strftime(
            "%Y-%m-%d"
        )  # 1998-09-02 / 1998-09-02 / 1998-09-02
    else:
    ...
    for i in tqdm(range(len(df)), desc="quarter_roll"):
        try:
            df["quarter_roll"][i] = int(
                (
                    pd.to_datetime(df["q"][i]).to_period(freq="Q")
                    - pd.to_datetime("1981Q1").to_period(freq="Q")
                ).n
            )
        except:
            continue
...
def bar_return(df, ax):
    ...
    fplt.volume_ocv(
        df[["open", "close", "bar_return"]], ax=ax, colorfunc=fplt.strength_colorfilter
    )
    """
    ...
    """
    df["daily_vol"] = (
        df.close.pct_change().ewm(span=60, min_periods=60).std().fillna(method="bfill")
    )
...
def target_return(df):
    ...
    df["target_return_3"] = (
        df.bar_return.shift(-3) + df.bar_return.shift(-2) + df.bar_return.shift(-1)
    )
    df["target_return_4"] = (
        df.bar_return.shift(-4)
        + df.bar_return.shift(-3)
        + df.bar_return.shift(-2)
        + df.bar_return.shift(-1)
    )
    df["target_return_5"] = (
        df.bar_return.shift(-5)
        + df.bar_return.shift(-4)
        + df.bar_return.shift(-3)
        + df.bar_return.shift(-2)
        + df.bar_return.shift(-1)
...
def plot_pinbar(df, period, ax):
    ...
    df["pin"] = (
        lo_wicks.ewm(span=period).mean() - hi_wicks.ewm(span=period).mean()
    ) * bar_range  # EMA
    if period > 0:
        df["pin"] = df["pin"].ewm(span=period).mean()  # double smoothed
    fplt.volume_ocv(
        df[["open", "close", "pin"]], ax=ax, colorfunc=fplt.strength_colorfilter
    )
...
def plot_twobar_pinbar(df, period, ax):
    ...
    fplt.volume_ocv(
        df[["open_shft", "close", "twobar_pin"]],
        ax=ax,
        colorfunc=fplt.strength_colorfilter,
    )  # , legend='pin_wicks_osc'
...
def plot_threebar_pinbar(df, period, ax):
    ...
    df["high_max"] = (
        pd.concat([df["high"], df["high"].shift(1), df["high"].shift(2)], axis=1)
        .T.max()
        .values
    )
    df["low_min"] = (
        pd.concat([df["low"], df["low"].shift(1), df["low"].shift(2)], axis=1)
        .T.min()
        .values
    )
    lo_wicks = df[["open_shft", "close"]].T.min() - df["low_min"]
    ...
    if period > 0:
        df["threebar_pin"] = (
            df["threebar_pin"].ewm(span=period).mean()
        )  # double smoothed
    fplt.volume_ocv(
        df[["open_shft", "close", "threebar_pin"]],
        ax=ax,
        colorfunc=fplt.strength_colorfilter,
    )  # , legend='pin_wicks_osc'
...
def plot_fourbar_pinbar(df, period, ax):
    ...
    df["high_max"] = (
        pd.concat(
            [df["high"], df["high"].shift(1), df["high"].shift(2), df["high"].shift(3)],
            axis=1,
        )
        .T.max()
        .values
    ...
    df["low_min"] = (
        pd.concat(
            [df["low"], df["low"].shift(1), df["low"].shift(2), df["low"].shift(3)],
            axis=1,
        )
        .T.min()
        .values
    ...
    fplt.volume_ocv(
        df[["open_shft", "close", "fourbar_pin"]],
        ax=ax,
        colorfunc=fplt.strength_colorfilter,
    )  # , legend='pin_wicks_osc'
...
def Import_FinBERT_Sentiment(
    df, folder: str, sub_folder: str, freq: str, cand_per_day: int, ax: str, axb: str
):
    """Import_FinBERT
    ...
    # 2 ---
    finbert_sentiment = pd.concat(list_of_files)[
        ["datePublished", "tweet", "pos_sent", "neg_sent", "neu_sent"]
    ]
    ...
    # 4 ---
    finbert_sentiment["datePublished"] = pd.to_datetime(
        finbert_sentiment["datePublished"]
    ...
    # 7 ---
    grouped = finbert_sentiment.groupby(
        pd.Grouper(key="timestamp_5day", axis=0, freq=freq)
    ).mean()  # use mean or sum
    # 8 ---
    ...
    df["finbert_sent_osc"] = df["pos_sent"].ewm(span=2).mean().rolling(
        cand_per_week
    ).apply(
        lambda x: np.polyfit(np.array(range(0, cand_per_week)), x, 1)[0], raw=True
    ) - df[
        "neg_sent"
    ].ewm(
        span=2
    ...
    fplt.volume_ocv(
        df[["open", "close", "finbert_sent_osc_day"]],
        ax=ax,
        colorfunc=fplt.strength_colorfilter,
    )
    fplt.volume_ocv(
        df[["open", "close", "finbert_sent_osc"]],
        ax=axb,
        colorfunc=fplt.strength_colorfilter,
    )
...
def Import_Economic_Calendar(df, folder: str, freq: str):  # , ax: str
    ...
    """
    US_econ_ind_list = (
        econ_cal[((econ_cal["Currency"] == "USD") & (econ_cal["Impact"] == "HIGH"))]
        .drop_duplicates("Name", keep="first")["Name"]
        .reset_index(drop=True)[[0, 1, 4, 8, 13, 34, 38, 68]]
        .tolist()
    )
    # print(US_econ_ind_list)

    # create new dataframe with just that list of economic indicators
    US_econ_cal = econ_cal[
        ((econ_cal["Currency"] == "USD") * (econ_cal["Name"].isin(US_econ_ind_list)))
    ][["timestamp_5day", "Name"]]
    # print(US_econ_cal)
    ...
    """
    EU_econ_ind_list = (
        econ_cal[((econ_cal["Currency"] == "EUR") & (econ_cal["Impact"] == "HIGH"))]
        .drop_duplicates("Name", keep="first")["Name"]
        .reset_index(drop=True)[[0, 3, 5, 10, 35]]
        .tolist()
    )
    # print(EU_econ_ind_list)

    # create new dataframe with just that list of economic indicators
    EU_econ_cal = econ_cal[
        ((econ_cal["Currency"] == "EUR") * (econ_cal["Name"].isin(EU_econ_ind_list)))
    ][["timestamp_5day", "Name"]]
    # print(EU_econ_cal)
    ...
    """
    GB_econ_ind_list = (
        econ_cal[((econ_cal["Currency"] == "GBP") & (econ_cal["Impact"] == "HIGH"))]
        .drop_duplicates("Name", keep="first")["Name"]
        .reset_index(drop=True)[[1, 4, 11]]
        .tolist()
    )
    # print(GB_econ_ind_list)

    # create new dataframe with just that list of economic indicators
    GB_econ_cal = econ_cal[
        ((econ_cal["Currency"] == "GBP") * (econ_cal["Name"].isin(GB_econ_ind_list)))
    ][["timestamp_5day", "Name"]]
    # print(GB_econ_cal)
    ...
    """
    JP_econ_ind_list = (
        econ_cal[((econ_cal["Currency"] == "JPY") & (econ_cal["Impact"] == "HIGH"))]
        .drop_duplicates("Name", keep="first")["Name"]
        .reset_index(drop=True)[[0, 2, 5]]
        .tolist()
    )
    # print(JP_econ_ind_list)

    # create new dataframe with just that list of economic indicators
    JP_econ_cal = econ_cal[
        ((econ_cal["Currency"] == "JPY") * (econ_cal["Name"].isin(JP_econ_ind_list)))
    ][["timestamp_5day", "Name"]]
    # print(JP_econ_cal)
    ...
    """
    CN_econ_ind_list = (
        econ_cal[((econ_cal["Currency"] == "CNY") & (econ_cal["Impact"] == "HIGH"))]
        .drop_duplicates("Name", keep="first")["Name"]
        .reset_index(drop=True)[[1, 2]]
        .tolist()
    )
    # print(CN_econ_ind_list)

    # create new dataframe with just that list of economic indicators
    CN_econ_cal = econ_cal[
        ((econ_cal["Currency"] == "CNY") * (econ_cal["Name"].isin(CN_econ_ind_list)))
    ][["timestamp_5day", "Name"]]
    # print(CN_econ_cal)
    ...
    """
    ALL_econ_cal = pd.concat(
        [US_econ_cal, EU_econ_cal, GB_econ_cal, JP_econ_cal, CN_econ_cal], axis=1
    ).reset_index()
    ALL_econ_cal.set_axis(
        [
            "timestamp_5day",
            "US_econ_cal",
            "EU_econ_cal",
    ...
    ALL_econ_cal["ALL_econ_cal"] = ALL_econ_cal[cols].apply(
    ...
    for i in tqdm(range(len(ALL_econ_cal)), desc="ALL_econ_cal"):
        ...
        nat_tz = tz.gettz(
            "Etc/UTC"
        )  # need to check this does not have native DST shift, otherwise use 'Europe/London'
        chart_tz = tz.gettz("Europe/Helsinki")
        dt = datetime(
            dt_native[i].year,
            dt_native[i].month,
    ...
    """
    ALL_econ_cal = ALL_econ_cal.groupby(
        pd.Grouper(key="timestamp_5day", axis=0, freq=freq)
    )
    ...
    """
    df = pd.concat(
        [df, ALL_econ_cal.reindex(df.index)], axis=1
    )  # THIS DOES NOT WORK <<<<<<<<<
...
def plot_vwap(df, period, colour, ax):
    ...
    for _, g in df.groupby(pd.Grouper(level="timestamp_5day", freq=period)):
        try:
            ...
            vwap = pd.concat(
                [vwap, g.hlc3v.loc[i0:i1].cumsum() / df.volume.loc[i0:i1].cumsum()],
                ignore_index=True,
            )
        except:
            continue
...
def plot_vwap_dist(df, vwap, ax):
    ...
    if vwap == "vwap_D":
        df.loc[
            ((df["high"] + df["low"] + df["close"]) / 3) > df[vwap], vwap + "_dist"
        ] = ((df["high"] + df["low"] + df["close"]) / 3) - df[vwap]
        df.loc[
            ((df["high"] + df["low"] + df["close"]) / 3) < df[vwap], vwap + "_dist"
        ] = ((df["high"] + df["low"] + df["close"]) / 3) - df[vwap]
    else:
...
def plot_ema_dist(df, ema, ax):
    ...
    fplt.volume_ocv(
        df[["open", "close", ema + "_dist"]], ax=ax, colorfunc=fplt.strength_colorfilter
    )
...
def chart_dataframe(df, ticker_name, new_timeframe):
    ...
    (
        ...
        axfin,
        axfin2,
    ) = fplt.create_plot(ticker_name, rows=18)
    ...
    fullpath = os.path.join(
        directory, experiment_name.replace(ticker_name.split("_")[0], "")[1:]
    )

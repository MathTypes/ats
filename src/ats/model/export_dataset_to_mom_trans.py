...
def import_data(
    ...
    if timeframe == "daily":
        start_date = (
            "2011-09-05"  # this gives one quarter of lead in before the training starts
        )
    ...
    """
    df["volume"] = (
        df["bar_return"].ewm(span=60, min_periods=60).std().fillna(method="bfill")
    )
    ...
    df = df[
        [  # mapped to the following finbert dataset features:
            "Date",  # Timestamp_5day

...
df = df.groupby("tic").apply(
    lambda x: x.assign(OpenPct=(x.Open.diff(1) / x.Open) * 10000)
)
df = df.groupby("tic").apply(
    lambda x: x.assign(ClosePct=(x.Close.diff(1) / x.Close) * 10000)
)
df = df.groupby("tic").apply(
    lambda x: x.assign(HighPct=(x.High.diff(1) / x.High) * 10000)
)
df = df.groupby("tic").apply(lambda x: x.assign(LowPct=(x.Low.diff(1) / x.Low) * 10000))
df = df.groupby("tic").apply(
    lambda x: x.assign(VolumePct=(x.Volume.diff(1) / x.Volume))
)
...
df["VolumePct"] = (
    df["VolumePct"].apply(lambda x: int(log(1 + max(x, -0.9)) * 10)).astype(np.float32)
)
...
logging.info(f"df:{df.info()}")
df = df.resample("5Min").agg(
    OpenPct=("OpenPct", "first"),
    HighPct=("HighPct", "first"),
    LowPct=("LowPct", "first"),
    ClosePct=("ClosePct", "first"),
    VolumePct=("VolumePct", "first"),
    Open=("Open", "first"),
    OpenCnt=("Open", "count"),
    High=("High", "max"),
    Low=("Low", "min"),

    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        for i, batch in enumerate(self.val_wrapper):
            ...
            if len(src_vec) == 64:
                ...
                prediction = (
                    inference.run_encoder_decoder_inference(
                ...
                for ind in top_ind:
                    ...
                    ax1.plot(
                        np.arange(close.shape[0] - 1, close.shape[0] + pred.shape[0]),
                        np.concatenate(([close[-1]], close[-1] + pred_close)),
                        label="Prediction",
                        color="red",
                    )
                    ax1.plot(
                        np.arange(close.shape[0] - 1, close.shape[0] + pred.shape[0]),
                        np.concatenate(([close[-1]], close[-1] + y_close)),
                        label="Groud Truth",
                        color="purple",
                    )
                    now = datetime.fromtimestamp(int(time.numpy()[-1])).strftime(
                        "%y-%m-%d %H:%M"
                    )
                ...
                for ind in top_ind:
                    ...
                    self.wandb_logger.log_image(
                        f"bottom-loss-chart-{now}", images=[fig]
                    )
                src_vec.clear()
                tgt_y_vec.clear()
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        for i in range(0, fig_cnt):
            ...
            for j in range(0, 4):
                ...
                ax1.plot(np.arange(close.shape[0]), close, label="Training data")
                ax1.plot(
                    np.arange(close.shape[0] - 1, close.shape[0] + pred_close.shape[0]),
                    np.concatenate(([close[-1]], pred_close)),
                    label="Prediction",
                    color="red",
                )
                ax1.plot(
                    np.arange(close.shape[0] - 1, close.shape[0] + pred_close.shape[0]),
                    np.concatenate(([close[-1]], y_close)),
                    label="Groud Truth",
                    color="purple",
                )

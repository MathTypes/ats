...
def get_trainer(config):
    ...
    strategy = RayStrategy(
        num_workers=config["num_workers"], num_cpus_per_worker=1, use_gpu=use_gpu
    )
    # configure network and trainer
...
def get_input_data(config):
    start = time.time()
    filter_expr = pds.field("hour_of_day").isin([3])
...
def run_train(config, net, trainer, train_dataloader, val_dataloader, data):
    ...
    for idx in range(4):  # plot 10 examples
        time_idx_val = val_dataloader.dataset.x_to_index(raw_predictions.x)["time_idx"][
            idx
        ]

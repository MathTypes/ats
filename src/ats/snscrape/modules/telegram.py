...
class TelegramChannelScraper(snscrape.base.Scraper):
    ...
    def _initial_page(self):
        if self._initialPage is None:
            ...
            self._initialPage, self._initialPageSoup = (
                r,
    ...
    def _soup_to_items(self, soup, pageUrl, onlyUsername=False):
        ...
        for post in reversed(posts):
            ...
            if (
                ...
                if (
                    descriptionDiv := linkPreviewA.find(
    ...
    def _get_entity(self):
        ...
        if (
            descriptionDiv := channelInfoDiv.find(
        ...
        def parse_num(s):
            ...
            if s.endswith("M"):
                return (
                    int(float(s[:-1]) * 1e6),
                    10 ** (6 if "." not in s else 6 - len(s[:-1].split(".")[1])),
                )
            elif s.endswith("K"):
                return (
                    int(float(s[:-1]) * 1000),
                    10 ** (3 if "." not in s else 3 - len(s[:-1].split(".")[1])),
                )
            else:
                return int(s), 1

...
def monthlist(begin, end):
    ...
    while True:
        ...
        if next_month > end:
            break
        result.append(
            [begin.strftime("%Y-%m-%d"), last_day_of_month(begin).strftime("%Y-%m-%d")]
        )
        begin = next_month
...
if __name__ == "__main__":
    ...
    with NitterScraper(host="0.0.0.0", port=args.port) as nitter:
        for user in users:
            for cur_date in monthlist(args.start_date, args.end_date):
                since = cur_date[0]
                until = cur_date[1]
                df = pd.DataFrame()
                query = f"search?f=tweets&q=from:{user}&until={until}&since={since}"
                try:
                    tweets = nitter.search_tweets(query, pages=10000)
                    for tweet in tweets:
                ...
                user_output_dir = os.path.join(args.output_dir, user)
                if not os.path.exists(user_output_dir):
                    os.mkdir(user_output_dir)
                df.to_csv(
                    user_output_dir + "/" + user + "_" + since + "_" + until + ".csv"
                )

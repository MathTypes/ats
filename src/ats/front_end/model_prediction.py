def render_model_prediction():
    ...
    def main(app_data):
        ...
        if asset in ["Index Fund", "Forex", "Futures & Commodities", "Stocks"]:
            ...
            if asset == "Futures & Commodities":
                ...
                currency = app_data.df_forex[
                    (app_data.df_forex["Currencies"] == equity)
                ]["Currency"].unique()[0]
                market = app_data.df_forex[(app_data.df_forex["Currencies"] == equity)][
                    "Market"
                ].unique()[0]
        ...
        risks = {
            ...
            "Medium": [
                analysis_day.df["S2"].values[-1],
                analysis_day.df["R2"].values[-1],
            ],
            "High": [
                analysis_day.df["S3"].values[-1],
                analysis_day.df["R3"].values[-1],
            ],
        }
        ...
        accuracy_threshold = {analysis.score_action: 75.0, analysis.score_price: 75.0}
        confidence = dict()

...
class MinerUtils:
    ...
    def get_miner_stats_df(self):
        ...
        spec = {
            ...
            "start_timestamp": (
                "markets",
                [("active_bounty_periods", ["start_timestamp"])],
            ),
            "end_timestamp": (
                "markets",
                [("active_bounty_periods", ["end_timestamp"])],
            ),
        ...
        df["exchange_coingecko_id"] = df.apply(
            lambda x: self.exchange_coingecko_id(x.exchange), axis=1
        )
        return df

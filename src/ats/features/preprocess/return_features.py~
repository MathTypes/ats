...
#@profile_util.profile
def example_level_features(group_features, cal:any,
                           macro_data_builder:any, config:dict) -> pd.DataFrame:
    raw_data = pd.DataFrame(raw_data)
    return example_level_features_df(cal=cal, macro_data_builder=macro_data_builder,
                                     config=config, raw_data=raw_data)

@profile_util.profile
#@njit(parallel=True)
def example_level_features_df(cal, macro_data_builder, config, raw_data):
    add_daily_rolling_features=config.model.features.add_daily_rolling_features
    ...
    raw_data["last_weekly_close_time"] = raw_data.timestamp.apply(
    ...
    raw_data["last_monthly_close_time"] = raw_data.timestamp.apply(
    ...
    raw_data["last_option_expiration_time"] = raw_data.timestamp.apply(

...
def getEventsCalendar(start_date, file_path):
    ...
    for row in rows:
        ...
        if 'calendar__row--grey' in row.get('class'):
            ...
            if impact == 'high' or impact == 'H':
                impact = '3'
            elif impact == 'medium' or impact == 'M':
                impact = '2'
            elif impact == 'low' or impact == 'L':
                impact = '1'
            else:
                impact = '0'
...
if __name__ == "__main__":
    today = datetime.now()
    year_future = int(today.year) + 1
    years = range(2007, year_future, 1)
    ...
    for y in years:
        for m in months:
            ...
            file_path = expanduser("~") + '/' + str(y) + '.' + str(strToNumMonth(m)) + '.01.csv'
...
# https://www.forexfactory.com/calendar?month=Aug.2023

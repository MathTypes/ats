...
class Keywords_api:
    def __init__(self, query: str, proxy: Union[str, None], tweets_count: int) -> None:
        self.query = query
        self.proxy = proxy
        self.tweets_count = tweets_count
        self.x_guest_key = ""
        self.authorization_key = "Bearer AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA"
        self.posts_data = {}
    ...
    def find_cursor(self, timeline):
        cursor = None
        try:
            cursor = timeline["instructions"][-1]["replaceEntry"]["entry"]["content"][
                "operation"
            ]["cursor"]["value"]
        except KeyError:
            cursor = timeline["instructions"][0]["addEntries"]["entries"][-1][
                "content"
    ...
    def scrap(self):
        try:
            self.x_guest_key = Scraping_utilities.find_x_guest_token(
            ...
            while len(self.posts_data) < self.tweets_count:
                ...
                if response:
                    ...
                    if cursor is None:
                        retry -= 1
                    logger.info(
                        "Number of Tweets scraped : {}".format(len(self.posts_data))
                    )
                elif retry <= 0:

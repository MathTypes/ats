...
exchange_in_sample = FutureExchange(
    in_sample,
    base_instrument="RMB",
    exclude_close=True,
...
exchange_out_sample = FutureExchange(
    out_sample,
    base_instrument="RMB",
    exclude_close=True,
    initial_balance=10000,
...
environment_in_sample = TradingEnvironment(
    exchange=exchange_in_sample,
    action_strategy=action_strategy,
    reward_strategy=reward_strategy,
    feature_pipeline=feature_pipeline,
)

environment_out_sample = TradingEnvironment(
    exchange=exchange_out_sample,
    action_strategy=action_strategy,
    reward_strategy=reward_strategy,
    feature_pipeline=feature_pipeline,
)

strategy_in_sample = TensorforceTradingStrategy(
    environment=environment_in_sample, agent_spec=agent_spec, save_best_agent=False
)

strategy_out_sample = TensorforceTradingStrategy(
    environment=environment_out_sample, agent_spec=agent_spec, save_best_agent=False
)
...
while episode < total_episodes:
    print("------in_sample------")
    performance = strategy_in_sample.run(
        episodes=(episode + out_sample_frequency), evaluation=False
    )
    fig = pd.DataFrame(performance[1].T).plot().get_figure()
    fig.savefig(
        "{}/in_sample_fig/{}.png".format(save_path, strategy_in_sample.agent.episodes)
    )
    fig.clf()
    strategy_in_sample.save_agent(directory=save_path, filename="agent")
    ...
    while trial < out_sample_trials:
        out_sample_nav = strategy_out_sample.run(
            episodes=(episode + trial + 1), evaluation=False
        )[1]
    ...
    numpy.save(
        "{}/out_sample_npy/{}.npy".format(save_path, num_out_sample), temp_ndarray
    )
...
in_sample_rewards.plot().get_figure().savefig(
    "{}/in_sample_rewards.png".format(save_path)
)
plt.cla()
pd.Series(out_sample_rewards).plot().get_figure().savefig(
    "{}/out_sample_rewards.png".format(save_path)
)
plt.cla()
pd.Series(out_sample_avrg_rewards).plot().get_figure().savefig(
    "{}/out_sample_avrg_rewards.png".format(save_path)
)
plt.cla()


performance = pd.DataFrame(
    data=performance.T, columns=("balance", "net_worth", "open_amount", "price")
)

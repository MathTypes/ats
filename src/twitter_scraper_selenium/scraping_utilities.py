...
class Scraping_utilities:
    ...
    def url_generator(
        ...
        if within_time is None:
            words = [
                Scraping_utilities.set_value_or_none(since, "since:"),
            ...
            word = Scraping_utilities.set_value_or_none(within_time, "within_time:")
            query = keyword + " " + word
        ...
        try:
            ...
            if proxy:
                ...
                response = requests.get(
                    URL, params=params, headers=headers, proxies=proxy_dict
                )
            else:
    ...
    def build_topic_headers(x_guest_token, authorization_key, rest_id=None):
        ...
        if rest_id:
            headers["referer"] = "https://twitter.com/i/topics/{}".format(rest_id)
        return headers

...
class Topic_api:
    ...
    def augment_data(self, data):
        try:
            for tweet in data:
                try:
                    typename = tweet["content"]["itemContent"]["tweet_results"][
                        "result"
                    ]["__typename"]
                except KeyError:
                    continue
        ...
        try:
            ...
            while len(self.data_dict) < self.tweets_count:
                ...
                response = Scraping_utilities.make_http_request_with_params(
                    graphql_link, params, headers, self.proxy
                )
                if response:
                    instructions = response["data"]["topic_by_rest_id"]["topic_page"][
                        "body"

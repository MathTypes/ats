...
class Technical_Calculations(Data_Sourcing):
    ...
    def moving_average_convergence_divergence(self):
        ...
        self.df["MACDS"] = (
            self.df["MACD"].ewm(span=self.signal_smoothing, adjust=False).mean()
        )
    ...
    def relative_strength_index(self):
        ...
        average_gain = gain.ewm(
            com=self.rsi_period - 1, min_periods=self.rsi_period
        ).mean()
    ...
    def slow_stochastic(self):
        ...
        fast_k = 100 * (
            (self.df["Adj Close"] - low_stochastic) / (high_stochastic - low_stochastic)
        )
    ...
    def moving_averages(self):
        ...
        self.df["SEMA"] = (
            self.df["Adj Close"]
            .ewm(com=self.short_run - 1, min_periods=self.short_run)
            .mean()
        )
        self.df["LMA"] = self.df["Adj Close"].rolling(self.long_run).mean()
        self.df["LEMA"] = (
            self.df["Adj Close"]
            .ewm(com=self.long_run - 1, min_periods=self.long_run)
            .mean()
        )
    ...
    def on_balance_volume(self):
        self.df["OBV"] = 0
        self.df.loc[
            ((self.df["Volume"].shift(-1)) < (self.df["Volume"])), "OBV"
        ] = self.df["OBV"] + self.df["Volume"].shift(-1)
        self.df.loc[
            ((self.df["Volume"].shift(-1)) > (self.df["Volume"])), "OBV"
        ] = self.df["OBV"] - self.df["Volume"].shift(-1)
        self.df.loc[((self.df["Volume"].shift(-1)) == (self.df["Volume"])), "OBV"] = (
            self.df["OBV"] + 0
        )
    ...
    def price_analysis(self):
        self.df["HL_PCT"] = (
            (self.df["High"] - self.df["Low"]) / self.df["Adj Close"] * 100.0
        )
        self.df["PCT_CHG"] = (
            (self.df["Adj Close"] - self.df["Open"]) / self.df["Open"] * 100.0
        )

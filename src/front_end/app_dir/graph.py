...
class Visualization(Prediction):
    ...
    def prediction_graph(self, equity=None):
        ...
        self.fig_action.update_layout(autosize=False, height=350, dragmode=False, hovermode='x', plot_bgcolor='rgba(255, 255, 255, 0.88)',
                                      title=dict(text=prediction_title, y=0.95, x=0.5,
                                                 xanchor='center', yanchor='top', font=dict(size=20)),
                                      xaxis_range=(self.df_visualization.index.min(
        ...
        self.fig_action.update_yaxes(
            title_text="Price & Trading Action", secondary_y=False, showgrid=False, showline=False)
    ...
    def technical_analysis_graph(self):
        ...
        ), x1=self.df_visualization_technical.index.max(), y0=20, y1=20, line=dict(color='#008000', width=1), row=2, col=1)
        self.fig_analysis.add_shape(type='line', x0=self.df_visualization_technical.index.min(
        ), x1=self.df_visualization_technical.index.max(), y0=80, y1=80, line=dict(color='#FF0000', width=1), row=2, col=1)
        ...
        self.fig_analysis.add_shape(type='line', x0=self.df_visualization_technical.index.min(
        ), x1=self.df_visualization_technical.index.max(), y0=20, y1=20, line=dict(color='#008000', width=1), row=4, col=1)
        self.fig_analysis.add_shape(type='line', x0=self.df_visualization_technical.index.min(
        ), x1=self.df_visualization_technical.index.max(), y0=80, y1=80, line=dict(color='#FF0000', width=1), row=4, col=1)
        ...
        self.fig_analysis.update_xaxes(
        ...
        self.fig_analysis.update_yaxes(
            title_text="Stochastic RSI", range=[-1, 101], tickvals=[0, 20, 80, 100], row=2, col=1)
        self.fig_analysis.update_yaxes(title_text="RSI", range=[0, 100], tickvals=[
                                       0, 30, 70, 100], row=3, col=1)
        self.fig_analysis.update_yaxes(
            title_text="Slow Stochastic", range=[-1, 101], tickvals=[0, 20, 80, 100], row=4, col=1)

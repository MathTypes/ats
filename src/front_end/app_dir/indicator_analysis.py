...
class Indications(Technical_Calculations):
    ...
    def engulfing_analysis(self):
        self.df.loc[
            (
                ...
                & (
                    self.df["Adj Close"].shift(self.engulfing_period)
                    > self.df["Open"].shift(self.engulfing_period)
                )
            ),
            "Engulfing_Indication",
        ] = 2
        self.df.loc[
            (
                (self.df["Adj Close"] > self.df["Open"])
    ...
    def moving_average_analysis(self):
        self.df.loc[
            (
                (self.df["SMA"] < self.df["LMA"])
                & (self.df["SMA"].shift(self.sma) > self.df["LMA"].shift(self.lma))
            ),
            "MA_Indication",
        ] = 0
        self.df.loc[
    ...
    def support_resistance(self):
        self.df.loc[
            (
                (self.df["SMA"] < self.df["Adj Close"])
                & (
                    self.df["SMA"].shift(self.sma)
                    > self.df["Adj Close"].shift(self.sma)
                )
            ),
            "Support_Resistance_Indication",
        ] = 0
        self.df.loc[
            (
                (self.df["SMA"] > self.df["Adj Close"])
    ...
    def price_action(self):
        ...
        self.df["Indication"] = (
            self.df.loc[:, "Engulfing_Indication":]
            .ewm(
                com=self.indication_estimate - 1,
        ...
        self.df.loc[
            ((self.df["Indication"] >= 1.25) & (self.df["Adj Close"] <= self.df["P"])),
            "Distinct_Action",
        ] = "Buy"
        self.df.loc[
            ((self.df["Indication"] <= 0.75) & (self.df["Adj Close"] >= self.df["P"])),
            "Distinct_Action",
        ] = "Sell"

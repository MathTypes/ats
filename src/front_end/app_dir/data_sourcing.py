...
def data_update_():
    ...
    if (
        (
            (dt.datetime.now() - pd.to_datetime(df_crypto["Last Update"][0])).days
            >= day_limit
        )
        or (
            (dt.datetime.now() - pd.to_datetime(df_stocks["Last Update"][0])).days
            >= day_limit
        )
        or (
            (dt.datetime.now() - pd.to_datetime(df_indexes["Last Update"][0])).days
            >= day_limit
        )
        or (
            (dt.datetime.now() - pd.to_datetime(df_futures["Last Update"][0])).days
            >= day_limit
        )
        or (
            (dt.datetime.now() - pd.to_datetime(df_forex["Last Update"][0])).days
            >= day_limit
        )
    ):
        update_market_data()

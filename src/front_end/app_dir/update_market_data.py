...
def _update_market_data():
    try:
        ...
        df_binance = pd.DataFrame(data["symbols"])[
            pd.DataFrame(data["symbols"])["status"] == "TRADING"
        ][["symbol", "baseAsset", "quoteAsset"]]
        df_binance = df_binance[
            (df_binance["quoteAsset"].isin(["BNB", "BTC", "BUSD", "ETH", "USDT"]))
        ]
    ...
    try:
        ...
        data = requests.get(url).json()
        df_binance_us = pd.DataFrame(data["symbols"])[
            pd.DataFrame(data["symbols"])["status"] == "TRADING"
        ][["symbol", "baseAsset", "quoteAsset"]]
        df_binance_us = df_binance_us[
            (df_binance_us["quoteAsset"].isin(["BTC", "BUSD", "ETH", "USD", "USDT"]))
        ]
    ...
    try:
        ...
        df_dow = pd.DataFrame(
            columns=["Ticker", "Company", "Index Fund", "Currency", "Currency_Name"]
        )
    try:
    ...
    try:
        ...
        df_csi.loc[df_csi["Stock exchange"] == "Shanghai", "Ticker"] = (
            df_csi["Index"].astype("str") + ".SS"
        )
        df_csi.loc[df_csi["Stock exchange"] == "Shenzhen", "Ticker"] = (
            df_csi["Index"].astype("str") + ".SZ"
        )
        tickers = []
        zeros = ["0"]
    ...
    try:
        ...
        df_forex["Currency"] = (
            df_forex["Currencies"].astype("str").apply(lambda x: x.split("/")[0])
        )
        df_forex["Market"] = (
            df_forex["Currencies"].astype("str").apply(lambda x: x.split("/")[1])
        )
        df_forex["Currencies"] = df_forex["Currencies"].apply(
    ...
    try:
        ...
        df_futures = df_futures.drop_duplicates(
            subset=["Ticker", "Futures"], keep=False
        )

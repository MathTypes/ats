...
def render_sentiment_analysis(
    ...
    analysis = st.sidebar.radio(
        "Choose analysis",
        ["Sentiment analysis", "Data exploration", "Aggregation charts"],
        index=0,
    )
    ...
    def draw_wordcloud(asset="all assets", start_date=None, end_date=None):
        ...
        if asset.lower() == "all assets":
            headlines100k = (
                news_df[news_df["time"].between(start_date, end_date)]["text"]
                .str.lower()
                .values[-100000:]
            )
        else:
    ...
    if analysis.lower() == "data exploration":
        ...
        assetNameGB = market_df[market_df["assetName"] == "Unknown"].groupby(
            "assetCode"
        )
        ...
        with row2_1:
            ...
            grouped = (
                market_df.groupby("time")
                .agg({"price_diff": ["std", "min"]})
                .reset_index()
            )
        ...
        start_date = st.sidebar.date_input(
            "Starting date", value=from_date, min_value=min_date, max_value=max_date
        )

        end_date = st.sidebar.date_input(
            "End date", value=to_date, min_value=min_date, max_value=max_date
        )
        ...
        if selected_assets:
            ...
            for asset in selected_assets:
                ...
                asset_df = asset_market_df[
                    asset_market_df["time"].between(start_date, end_date)
                ]
        ...
        def trends_quantile_chart(market_df):
            ...
            for i in [0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95]:
                ...
                price_df = market_df.groupby("time")["close"].quantile(i).reset_index()
                data.append(
                    go.Scatter(
        ...
        assets = assetNames
        selected_assets = st.sidebar.multiselect(
            "Please select the assets", assets, default=["ES"]
        )
        ...
        def calculate_mean_sentiment(asset, period, analyst_rating):
            ...
            asset_news_df = news_df[
                (news_df["assetName"] == asset)
        ...
        def altair_histogram(hist_data, sentiment_data):
            ...
            x_init = pd.to_datetime(
                [hist_data["eventTime"].min(), hist_data["eventTime"].max()]
            ).astype(int)
            logging.info(f"x_init:{x_init}")
            brush = alt.selection(
                type="interval", encodings=["x"], init={"x": list(x_init)}
            )
        ...
        sentiment_df["eventTime"] = sentiment_df["eventTime"].apply(
            lambda x: x.timestamp() * 1000
        )
        sentiment_df = sentiment_df.set_index(["time"])
        sentiment_df = sentiment_df.sort_index()
